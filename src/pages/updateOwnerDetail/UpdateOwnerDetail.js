...
const UpdateOwnerDetail = () => {
    ...
    const fetchUserId = async () => {
        ...
        try {
          ...
          const owner = response.data.find(owner => owner.user.username === username);
          if (owner) {
            setUserId(owner.id);
          }
        } catch (error) {
   ...
   const handleUpdate= async (e)=>{
        ...
        try {
            const response = await axios.patch(`https://merchandise-edt-les-crossing.trycloudflare.com/register/hospital_owner/${userId}/`,payload,{
                headers: {
